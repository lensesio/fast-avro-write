...
object FastWriteProgram extends App with Timed {
  ...
  AvroFileWriter.fastWrite(files.last, recordsCount, 8, schema, records)
  val stats = (1 to runs).map(files).map { f =>
    withTime {
      AvroFileWriter.fastWrite(f, recordsCount, 8, schema, records)
    }
  }
